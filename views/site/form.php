...
<?= $form->field($model, 'testing_type')
    ->dropDownList([Question::humanTestingType()])
    ->label("Тип тестового задания") ?>
<?= $form->field($model, 'content')->textarea()->label("Содержание вопроса") ?>
<?= $form->field($model, 'answer_options1')->textarea(['rows' => 1])->label("Варианты ответа (через запятую)") ?>
<?= $form->field($model, 'answer_options2')->textarea(['rows' => 1])
